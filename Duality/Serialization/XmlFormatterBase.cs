...
namespace Duality.Serialization
{
	public abstract class XmlFormatterBase : FormatterBase
	{
		...
		{
			this.writer.WriteString(obj);
		}

		protected void WriteArrayData(bool[] obj)
		{
			MemoryStream tempStream = new MemoryStream(obj.Length);
			using (BinaryWriter tempWriter = new BinaryWriter(tempStream))
			{
				for (long l = 0; l < obj.Length; l++) tempWriter.Write(obj[l]);

				tempWriter.Flush();
				this.writer.WriteBinHex(tempStream.ToArray(), 0, (int)tempStream.Length);
			}
		}
		protected void WriteArrayData(byte[] obj)
		{
			this.writer.WriteBinHex(obj, 0, obj.Length);
		}
