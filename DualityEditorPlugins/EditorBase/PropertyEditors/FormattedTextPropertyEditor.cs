...
namespace EditorBase.PropertyEditors
{
	public class FormattedTextPropertyEditor : MemberwisePropertyEditor
	{
		...
		{
			if (ReflectionHelper.MemberInfoEquals(info, ReflectionInfo.Property_FormattedText_DisplayedText)) return false;
			if (ReflectionHelper.MemberInfoEquals(info, ReflectionInfo.Property_FormattedText_Elements)) return false;
			return base.MemberPredicate(info);
		}
		protected override PropertyEditor MemberEditor(MemberInfo info)
		{
			if (ReflectionHelper.MemberInfoEquals(info, ReflectionInfo.Property_FormattedText_Icons) ||
				ReflectionHelper.MemberInfoEquals(info, ReflectionInfo.Property_FormattedText_FlowAreas) ||
				ReflectionHelper.MemberInfoEquals(info, ReflectionInfo.Property_FormattedText_Fonts))
			{
